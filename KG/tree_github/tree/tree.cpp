...
// Specifies number of leaf objects (points) to be displayed & colonized
    ...
    double branchX, branchY, branchZ;
...
void Tree::grow()
{
    ...
    double branchX, branchY, branchZ;	// Coords for base of branch
    ...
    for (int i = 0; i < allLeaves.size(); i++) {
        ...
        if (closestBranch != nullptr) {
            ...
            vector<double> newDir = {leafX - branchX, leafY - branchY, leafZ - branchZ};		// Vector from base of branch to leaf
            ...
            vector<double> resultantDir = {newDir[0] + closestDir[0], newDir[1] + closestDir[1], newDir[2] + closestDir[2]};
